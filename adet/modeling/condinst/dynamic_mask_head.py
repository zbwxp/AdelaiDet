...
def compute_point_term(pred_logits, pred_inst, gt):
    pred_inst_inds = pred_inst.gt_inds
    point_coords = torch.cat([per_im.point_coords for per_im in gt])[pred_inst_inds]
    point_labels = torch.cat([per_im.point_labels for per_im in gt])[pred_inst_inds]
    ...
    return roi_mask_point_loss(mask_logits, gt, point_labels)
